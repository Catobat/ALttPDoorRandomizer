...
def link_overworld(world, player):
    ...
    edges_to_swap = [edge for edge in swapped_edges if edge not in crossed_edges] + [edge for edge in crossed_edges if edge not in swapped_edges]
    if world.owShuffle[player] == 'vanilla':
        ...
        # layout shuffle
        groups = adjust_edge_groups(world, player, trimmed_groups, edges_to_swap)
...
def shuffle_tiles(world, groups, result_list, for_grouped, player):
    ...
    while True:
        ...
        if (world.owCrossed[player] == 'none' or for_grouped) and parity[:4] != [0, 0, 0, 0]:
            attempts -= 1
            continue
        # ensure sanc can be placed in LW in certain modes
        if not for_grouped and world.shuffle[player] not in ['vanilla', 'dungeonssimple', 'dungeonsfull', 'lean', 'crossed', 'insanity'] and world.mode[player] != 'inverted' and (world.doorShuffle[player] != 'crossed' or world.intensity[player] < 3 or world.mode[player] == 'standard'):
...
def define_tile_groups(world, player, for_grouped):
    groups = [[i, i + 0x40] for i in range(0x40)]

    def get_group_with(id):
        for group in groups:
            if id in group:
                return group
    ...
    def can_shuffle_group(group):
        ...
        if world.mode[player] == 'standard' and (0x1b in group or 0x2b in group or 0x2c in group):
            return False
        # sanctuary screen should not be swapped in some modes
        if 0x13 in group and (world.shuffle[player] in ['vanilla', 'dungeonssimple', 'dungeonsfull'] or (world.shuffle[player] == 'lite' and world.mode[player] == 'inverted')) and (world.doorShuffle[player] != 'crossed' or world.intensity[player] < 3 or world.mode[player] in ['standard', 'inverted']):
            return False
        return True

    for i in [0x00, 0x03, 0x05, 0x18, 0x1b, 0x1e, 0x30, 0x35]:
        groups.remove(get_group_with(i + 1))
        groups.remove(get_group_with(i + 8))
        groups.remove(get_group_with(i + 9))
    groups.append([0x80])
    groups.append([0x81])
    ...
    if world.owShuffle[player] == 'vanilla' and (world.owCrossed[player] == 'none' or for_grouped):
    ...
    if not world.owWhirlpoolShuffle[player] and (world.owCrossed[player] == 'none' or for_grouped):
...
def adjust_edge_groups(world, player, trimmed_groups, edges_to_swap):
                ...
                for i in range(2):
                    for edge_set in group[i]:
                        ...
                        if edge_set[0] in edges_to_swap:
                            new_world += 1
                        groups[(mode, new_world % 2, dir, terrain, parallel, count)][i].append(edge_set)
